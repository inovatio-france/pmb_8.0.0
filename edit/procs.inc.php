...
// +-------------------------------------------------+
// $Id: procs.inc.php,v 1.71.2.3 2024/05/24 08:17:38 dgoron Exp $

if (stristr($_SERVER['REQUEST_URI'], ".inc.php")) die("no access");

global $class_path, $msg, $charset, $categ, $sub, $current_module, $pmb_set_time_limit;
global $dest, $force_exec, $form_type, $limite_page, $nombre_lignes_total, $numero_page, $id_proc, $sort;
		...
		if ($nbr_lignes) {
			switch($dest) {
					...
					for($i=0; $i < $nbr_lignes; $i++) {
						...
						foreach($row as $col) {
							...
							if (is_numeric($col)) {
								...
								$worksheet->write_string(($i+2),$j,$col);
							}
							$j++;
						}
					}
				...
				default:
					...
					for($i=0; $i < $nbr_champs; $i++) {
						$fieldname = pmb_mysql_field_name($res, $i);
						print "<th class='align_left' onMouseOver ='survol(this);' onClick='sort_by_col(".(!is_object($sort) && $sort==($i+1)?(-($i+1)):($i+1)).");'>".$fieldname;
						if(!is_object($sort)) {
    						if($sort==($i+1)){
    							print "&nbsp;&#x25B4;";
    						}elseif((-$sort)==($i+1)){
					...
					<input type='hidden' id='sort' name='sort' value='".(!is_object($sort) ? $sort : '')."' />";
					print $param_hidden;
