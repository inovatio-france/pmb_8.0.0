...
// +-------------------------------------------------+
// $Id: artecampus.class.php,v 1.1.2.2 2024/09/05 13:05:00 qvarin Exp $
...
class artecampus extends connector
{
    ...
    protected function fetch_uai(int $location, int $source_id)
    {
    	...
    	$artecampus_locations = isset($parameters['artecampus_locations']) ? $parameters['artecampus_locations'] : [];
    ...
    {
    	$parameters = $this->fetch_parameters($source_id);
    	$artecampus_roles = isset($parameters['artecampus_roles']) ? $parameters['artecampus_roles'] : [];
    ...
    {
    	$parameters = $this->fetch_parameters($source_id);
    	$artecampus_key = isset($parameters['artecampus_key']) ? $parameters['artecampus_key'] : '';
    ...
    /**
     * M.A.J. Entrepot lie à une source
     *
    ...
    public function maj_entrepot($source_id, $callback_progress="", $recover=false, $recover_env="")
    {
        ...
        $parameters = $this->fetch_parameters($source_id);
        $artecampus_url = isset($parameters['artecampus_url']) ? $parameters['artecampus_url'] : '';
    ...
    protected function get_unimarc_record($record)
    {
        ...
        // Date de publication
        if (!empty($record['editorial']['publicationDate'])) {
            $unimarc['610'][0]['a'][0] = $record['editorial']['publicationDate'];
        }

        // Duree en minutes
        if (!empty($record['technical']['duration'])) {
            $unimarc['610'][0]['a'][0] = $record['editorial']['duration'];
        }
        // Annee de production
        if (!empty($record['technical']['productionYear'])) {
            $unimarc['100'][0]['a'][0] = $record['editorial']['productionYear'];
        }
        // Langue
        if (!empty($record['technical']['languages'])) {
            foreach ($record['technical']['languages'] as $lang) {
                $unimarc['101'][] = [
                    'a' => [$lang],
                ];
            }
    ...
    public function source_get_property_form($source_id)
    {
        ...
        $artecampus_key = isset($parameters['artecampus_key']) ? $parameters['artecampus_key'] : '';
        $artecampus_locations = isset($parameters['artecampus_locations']) ? $parameters['artecampus_locations'] : [];
        $artecampus_location_ids = isset($parameters['artecampus_location_ids']) ? $parameters['artecampus_location_ids'] : '';
        $artecampus_roles = isset($parameters['artecampus_roles']) ? $parameters['artecampus_roles'] : [];
